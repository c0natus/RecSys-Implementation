...
class Diffusion():
    def __init__(self, noise_steps=1000, beta_start=1e-4, beta_end=0.02, img_size=32, device='cuda'):
        ...
        # Step이 0일 때는 beta가 필요 없기 때문에 beta[0]은 beta_1의 값이다. 
        # 따라서 sample_steps에서 생성된 noise_steps은 실제론 noise_steps + 1이라고 생각하면 된다.
    ...
    def sample_timesteps(self, batch_size: int) -> torch.tensor:
        ...
        """
        return torch.randint(low=0, high=self.noise_steps, size=(batch_size,)) # 0 ~ noise_steps - 1 사이의 값 sampling.
    ...
    def sample_new_image(self, model, n):
        ...
        with torch.no_grad():
            ...
            for idx in reversed(range(self.noise_steps)):
                ...
                if idx > 0: noise = torch.randn_like(x)
